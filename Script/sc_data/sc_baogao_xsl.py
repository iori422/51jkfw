...
def Report():
    ...
    for i in range(aa):
        ...
        code = pp[2]
        phone =pp[3]
        sheet1_data =[ "1",chickID,chickID,code,name,"",sex,"19701216","24"
                 ,"",phone,"20170311","20170213",u"综述,综述综述","","","P121702050028"] #体检报告信息
        sheet1_loog =  len(sheet1_data)
        for a in  range(len(sheet1_data)):
            p = sheet1_data[a];
            sheet1.write(i+1,a,sheet1_data[a],staly_1)
    ...
    for i in range(aa):
         ...
         item_unit=pp[3]
         item_value=pp[4]
         # print item_number,item_name,item_unit,item_value,item_range
         result = '62'  #项目值
         code = ident_generator()
         sheet3_data = ['1',chickID,u"A709","0024","0024",item_number,item_number,
                   item_name,item_unit,item_range,u"XXXXXXXXXXXXXXXXXXXX测试",item_value,item_value,"",""]#体检检查项目表
         for a in range(len(sheet3_data)):
              p = sheet3_data[a]
    ...
    sheet6_data = ["1",chickID,u"肥胖症",u"建议：忌烟酒","G215"
        ,u""]#疾病诊断与阳性筛查
    sheet6_loog =  len(sheet6_data)
    for a in  range(len(sheet6_data)):
        p = sheet6_data[a];
